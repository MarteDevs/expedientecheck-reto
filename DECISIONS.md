...
**Problema:** Se intentó migrar la aplicación para consumir un dataset multianual. Sin embargo, el endpoint `datastore_search_sql` devolvía errores HTTP 500 consistentes.
**Decisión:** Se decidió revertir la migración y mantener la aplicación conectada al dataset "2024-Gasto", el cual ha demostrado ser estable.
**Beneficio:** Asegurar la disponibilidad de la aplicación, priorizando la estabilidad del sistema sobre la cantidad de datos históricos.
...
**Decisión:** Se implementó etiquetado especial para filas donde `Devengado > 0` y `PIM = 0` (badge "Gasto sin PIM"), y se eliminó la barra de porcentaje rota en el detalle individual.

## 7. Filtro por Mes de Ejecución (`MES_EJE`)
**Problema:** Al cargar los datos sin filtro de mes, la tabla mezclaba registros de todos los meses, dificultando el análisis.
**Decisión:** Se agregó un cuarto filtro dropdown para seleccionar el mes de ejecución (Valores 1-12 estáticos).
**Beneficio:** Permite análisis temporal sin necesidad de agregaciones SQL (que la API no soporta en este endpoint).
